...
vozInput.addEventListener('change', () => {
    ...
    if (matchMarcadores) {
        ...
        console.log("Item adicionado (Marcadores):", novoItem); // Debug log
        atualizarLista();
        salvarDados();
        vozInput.value = '';
        ...
        const novoItem = { descricao, quantidade, valor, categoria };
        compras.push(novoItem);
        console.log("Item adicionado (Natural):", novoItem); // Debug log
        atualizarLista();
        salvarDados();
        vozInput.value = '';
        vozFeedback.textContent = '';
...
function atualizarLista(filtrados = compras) {
    ...
    filtrados.forEach((item, index) => {
        const li = document.createElement('li');
        li.textContent = `${item.quantidade}x ${item.descricao} - R$ ${(item.valor).toFixed(2).replace('.', ',')} (${item.categoria})`; // Display total item value
        li.classList.add('fade-in');
...
salvarEdicaoBtn.addEventListener('click', () => {
    if (itemEditandoIndex !== null) {
        ...
        if (novoValor <= 0) {
            alert('Valor total inválido. Insira um valor maior que zero.');
            return;
        }
...
function carregarDados() {
    ...
    compras = JSON.parse(localStorage.getItem('compras')) || []; // Ensure compras are loaded on startup
