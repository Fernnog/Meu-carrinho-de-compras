...
// ***** NOVA FUNÇÃO processarEAdicionarItem (Refinada com Marcadores) *****
function processarEAdicionarItem(textoOriginal) {
    ...
    // Inclui \b para garantir que sejam palavras inteiras (evita match em 'descanso')
    // Inclui \s* para consumir espaços após o marcador
    const todosMarcadoresRegex = new RegExp(`\\b(${
        [...marcadores.quantidade, ...marcadores.descricao, ...marcadores.preco].join('|')
    })\\b\\s*`, 'gi'); // 'g' para global, 'i' para case-insensitive

    // 2. Dividir o texto usando os marcadores como delimitadores
    // A regex captura os marcadores, então split os inclui no array resultante
    // Ex: "qtd 2 desc arroz preco 3,99" -> ["", "qtd", " 2 ", "desc", " arroz ", "preco", " 3,99"] (aproximadamente)
    const partes = texto.split(todosMarcadoresRegex).filter(p => p && p.trim() !== ''); // Filtra partes vazias ou só com espaços

    let tipoAtual = null; // Para saber a qual marcador o valor seguinte pertence
    let descricaoColetada = []; // Para coletar partes da descrição

    // Se a primeira parte NÃO é um marcador conhecido, assume-se que é o início da descrição
    if (partes.length > 0 && !Object.values(marcadores).flat().includes(partes[0].toLowerCase().trim())) {
        tipoAtual = 'descricao'; // Inicia assumindo descrição
        descricaoColetada.push(partes[0].trim()); // Adiciona a primeira parte
    }
    ...
    for (let i = 0; i < partes.length; i++) {
        ...
        for (const [tipo, keywords] of Object.entries(marcadores)) {
            if (keywords.includes(parteLower)) {
                // É um marcador! Define o tipo para a PRÓXIMA parte
                tipoAtual = tipo;
                marcadorEncontrado = true;
                break; // Sai do loop de marcadores
            }
        }

        // Se NÃO for um marcador, é um valor ou continuação da descrição
        if (!marcadorEncontrado && parteAtual) {
            switch (tipoAtual) {
                case 'quantidade':
                    // Tenta extrair número (pode ser por extenso - *não implementado*, foca no numérico)
                    const qtdNum = parseInt(parteAtual.match(/\d+/)?.[0] || 'NaN', 10); // Pega só o número
                    if (!isNaN(qtdNum)) {
                        quantidade = qtdNum;
                    } else {
                        // Poderia tentar converter extenso para número aqui se necessário
                        console.warn(`Não foi possível parsear quantidade: "${parteAtual}"`);
                    }
                    // tipoAtual = null; // Reseta? Ou espera próximo marcador? Melhor esperar.
                    break;
                case 'preco':
                    valorUnitario = parseNumber(parteAtual); // Usa a função existente
                    // tipoAtual = null;
                    break;
                case 'descricao':
                    // Adiciona à descrição (pode ser multi-partes)
                    descricaoColetada.push(parteAtual);
                    // Mantém tipoAtual como 'descricao' até encontrar outro marcador
                    break;
                default:
                    // Se não há um tipo definido (ex: texto antes do primeiro marcador),
                    // ou se o tipo anterior já foi processado (embora não estejamos resetando),
                    // assume que é parte da descrição.
                    if (!tipoAtual) { // Garante que texto sem marcador precedente vá para descrição
                         descricaoColetada.push(parteAtual);
                    }
                    break;
            }
             // Se o valor foi processado (quantidade ou preço), podemos resetar o tipo
             // para evitar que o próximo bloco de texto sem marcador seja atribuído a eles.
             // A descrição é a única que pode continuar.
             if (tipoAtual === 'quantidade' || tipoAtual === 'preco') {
                 tipoAtual = null; // Reseta após processar valor de qtd/preco
             }
        }
    }
    ...
    if (!descricao) {
        // Se após tudo isso a descrição estiver vazia, mas tínhamos texto original,
        // usa o texto original como descrição (caso só tenha digitado o nome)
    ...
    // --- Lógica de Adicionar/Atualizar Item (Idêntica à versão anterior do script) ---
    try {
        ...
        if (itemExistenteIndex > -1) {
            // Item já existe. Confirma atualização.
             if (confirm(`"${descricao}" já está na lista com quantidade ${compras[itemExistenteIndex].quantidade}. Deseja somar a nova quantidade (${quantidade}) e atualizar o valor unitário (para R$ ${valorUnitario.toFixed(2).replace('.', ',')})?`)) {
                ...
                // Atualiza valor unitário se um novo foi fornecido E é maior que zero,
                // OU se o valor antigo era zero/nulo.
                if (valorUnitario > 0 || !compras[itemExistenteIndex].valorUnitario || compras[itemExistenteIndex].valorUnitario <= 0) {
                 ...
                 // Se a descrição mudou significativamente, talvez devesse re-inferir? Vamos manter simples por enquanto.
            } else {
            ...
            // Item novo
            const categoria = inferirCategoria(descricao); // Inferencia de categoria mantida
...
if (SpeechRecognition) {
    ...
    recognition.onresult = (event) => {
        ...
        if (targetInput) {
            ...
            // Descomentar a linha abaixo se quiser adicionar o item automaticamente após o ditado no input principal
